...
| 局限 | 该修正**不是独立或前瞻验证**（manuscript L275-277 已声明）；是观察失配后对判据的追溯调整。pre-registration 时间戳证据不存在，故全文用 "original declared criterion" 而非 "pre-registered"（修正记录见 [ENV_RECON_AND_MANUSCRIPT_FIX_2026-09-21.md](evidence/ENV_RECON_AND_MANUSCRIPT_FIX_2026-09-21.md) §3 M1/M4/M8-M10/M15/M17/M19 与 README R1） |
