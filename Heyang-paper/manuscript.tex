...
%%   ../WS-1_dataset/R6_recalc/results/comparison_linux_full764.json
%% (the Linux rerun vs the reference onboard results)
...
three dose levels was recomputed by an independent operator in a separate environment --- Linux
(Ubuntu 24.04 LTS) instead of Windows, with platform-specific CUDA/cuDNN builds at the same
nominal PyTorch 2.3.0+cu121 stack --- from checkpoints and data
...
\noindent\textbf{Results:} The original declared criterion failed for two of five methods
(RED-CNN, CoreDiff). In the earlier Windows same-OS recomputation, re-running the affected methods
with deterministic kernels reproduced the original bit-for-bit, and no kernel non-determinism was
observed in that environment. The criterion audit showed that none of
...
difference of $10^{-6}$ applied uniformly. Two of five methods fell outside it. The obvious
...
run was overwritten. The recomputation environment was Ubuntu 24.04.3 LTS (WSL2) with Python 3.12.3,
PyTorch 2.3.0+cu121, and NumPy 1.26.4, on the same two NVIDIA RTX 4090 GPUs (driver 591.86) exposed
through WSL passthrough, with platform-specific cuDNN 8.9.2.26. The reference environment,
...
a re-seed tests the pipeline's internal determinism, whereas a different library stack --- here,
Linux (Ubuntu 24.04 LTS) instead of Windows, with platform-specific CUDA/cuDNN builds at the
same nominal PyTorch 2.3.0+cu121 stack (Table~\ref{tab:env}) --- tests whether the published number
is a property of the method or of the machine.

\begin{table}[ht]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
Field & Reference / recalc (Windows) & Independent re-run (Linux) \\
\midrule
OS & Windows & Ubuntu 24.04.3 LTS (WSL2) \\
GPU & 2$\times$ NVIDIA RTX 4090 & 2$\times$ NVIDIA RTX 4090 (WSL passthrough) \\
GPU driver & 591.86 & 591.86 (WSL passthrough) \\
Python & 3.12.10 & 3.12.3 \\
PyTorch & 2.3.0+cu121 & 2.3.0+cu121 \\
torchvision & 0.18.0+cu121 & 0.18.0+cu121 \\
cuDNN & not separately recorded & 8.9.2.26 (nvidia-cudnn-cu12) \\
NumPy & 1.26.4 & 1.26.4 \\
SciPy & 1.13.1 & 1.13.1 \\
scikit-image & 0.23.2 & 0.23.2 \\
tifffile & 2024.9.20 & 2026.3.3 \\
mkl & 2021.4.0 (SHA-256-verified wheel) & --- \\
Dependency freeze & 57-line \texttt{env\_pip\_freeze.txt} & \texttt{pip freeze} of \texttt{.venv\_r6} \\
Source baseline & commit \texttt{4875338} & commit \texttt{b0686eb} (editable) \\
Re-run date & --- & 2026-09-23/24 \\
\bottomrule
\end{tabular}
\caption{\textbf{Computing environment record.} The reference environment --- the local run
environment, by the owner's decision --- is recorded in the left column; the recomputation
environment --- an independent Linux re-run on 2026-09-23/24, with all 375 comparisons within the
relative tolerance $10^{-4}$ --- is recorded in the right column. The complete 57-line freeze and
the hashes are committed under \texttt{WS-1\_dataset/R6\_recalc/hashes/}.}
\label{tab:env}
\end{table}
...
We re-ran the three methods outside the original declared criterion --- RED-CNN, CTformer and
CoreDiff --- under the following settings, at a cost of approximately 48 GPU-hours. This ablation
was performed during the earlier Windows same-OS recomputation (2026-09-05/06); the independent
Linux rerun was then compared directly against the reference, under which CTformer already
reproduced within the criterion and only RED-CNN and CoreDiff remained outside:
...
Table~\ref{tab:agreement} reports the comparison. Under the original declared absolute criterion, 90
of 375 comparisons differed. The blur control, LEARN and CTformer reproduced within the criterion
(for CTformer the largest absolute difference was $2.9\times10^{-11}$); RED-CNN and CoreDiff did
not.
...
criterion of $10^{-4}$, which is zero everywhere. That three methods reproduce exactly, together with
the route-(a) rerun of Section~\ref{sec:determinism}, shows that the pipeline, data and code
snapshot are capable of bit-identical reproduction under this configuration; in that earlier
Windows environment the differences in the other two were not observed to depend on kernel
non-determinism.}
\label{tab:agreement}
\end{table}

The largest single disagreement is instructive. For RED-CNN the worst absolute difference is 34.26,
...
zero differing comparisons, bit-identical. This ablation was performed during the earlier Windows
same-OS recomputation (2026-09-05/06), and no kernel non-determinism was observed in that
...
also fails at $10^{-5}$, $10^{-4}$ and $10^{-3}$, with the same two methods (RED-CNN and CoreDiff)
outside at every level. Loosening an absolute tolerance by three orders of magnitude changes nothing
within the range tested, because the
...
A relative criterion behaves differently. It excludes the same two methods (RED-CNN and CoreDiff)
at $10^{-6}$ and $10^{-5}$ and admits all five at $10^{-4}$.
...
across environments or particular to this pair, and this remains the clearest extension. A third
environment is planned; its results are not reported here.
