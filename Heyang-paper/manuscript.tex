...
Dependency freeze & 57-line \texttt{env\_pip\_freeze.txt} \\
Source baseline & commit \texttt{4875338} \\
\bottomrule
\end{tabular}
\caption{\textbf{Computing environment record.} The recomputation environment and the reference
environment --- the local run environment, by the owner's decision --- are both covered by this
single project-runtime record; the complete 57-line freeze and the hashes are committed under
\texttt{WS-1\_dataset/R6\_recalc/hashes/}.}
\label{tab:env}
\end{table}
