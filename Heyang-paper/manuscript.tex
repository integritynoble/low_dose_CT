...
verified by hash. Agreement was assessed against the benchmark's pre-registered criterion, an
...
\noindent\textbf{Results:} The pre-registered criterion failed for three of five methods. The
determinism hypothesis was falsified: re-running the same side with deterministic kernels reproduced
the original bit-for-bit, leaving the residual attributable to the environment rather than to kernel
non-determinism. The criterion audit showed that no absolute tolerance resolves the comparison ---
it fails at $10^{-6}$ and equally at $10^{-3}$ --- because the reported quantities span from a
...
differing scale, and loosening it does not repair it; only changing its kind does. We recommend that
...
rather than terminal. The pre-registered criterion of the benchmark under study was an absolute
...
the difference was not caused by the mechanism everyone expects, and the criterion could not have
been satisfied by any absolute threshold worth declaring.
...
with an eye filter). Summary statistics and the number of contributing slices give
\todo{state the count: 5 methods $\times$ 3 dose levels $\times$ 5 reported quantities = 75 comparisons per method}
per method, 375 in total.
...
differed in operating system, CUDA build, and framework version.
\todo{Record the reference environment exactly --- OS, driver, CUDA, torch, numpy. The recalculation
report characterises it only as ``the author's environment'', which is not enough for a paper whose
subject is cross-environment agreement. This is the single most important gap to close before
submission.}
...
selection. This hypothesis is testable and it makes a sharp prediction: with deterministic kernels
enabled, a re-run of the same side must be bit-identical, and any residual against the reference
must therefore come from somewhere else.

We re-ran the three methods outside the pre-registered criterion with
\texttt{cudnn.deterministic=True}, \texttt{cudnn.benchmark=False},
\texttt{torch.use\_deterministic\_algorithms(True)}, and
\texttt{CUBLAS\_WORKSPACE\_CONFIG=:4096:8}, at a cost of approximately 48 GPU-hours.
...
Table~\ref{tab:agreement} reports the comparison. Under the pre-registered absolute criterion, 115
...
pre-registered absolute $10^{-6}$ criterion. The worst relative difference and its metric are given
for each method; the final column counts comparisons outside the declared per-metric relative
criterion of $10^{-4}$, which is zero everywhere. That two methods reproduce exactly establishes
that the pipeline, data and code snapshot are capable of bit-identical reproduction, so the
differences in the other three are a property of those computations rather than of the harness.}
\label{tab:agreement}
\end{table}
...
\subsection{The determinism hypothesis is false here}

The prediction of Section~\ref{sec:determinism} failed. Re-running the recomputation side with
deterministic kernels reproduced the non-deterministic run exactly: zero differing comparisons,
bit-identical. The evaluation path was not sensitive to kernel non-determinism in this environment,
so route (a) as scoped could not close the residual, and the difference against the reference is a
systematic cross-environment offset.
...
\subsection{No absolute tolerance resolves the comparison}

Table~\ref{tab:ladder} is the central result. The pre-registered criterion fails at $10^{-6}$; it
also fails at $10^{-5}$, $10^{-4}$ and $10^{-3}$, with the same three methods outside at every
level. Loosening an absolute tolerance by three orders of magnitude changes nothing, because the
...
measurement changed. An absolute criterion cannot be repaired by loosening it; only changing its
kind resolves the comparison.}
\label{tab:ladder}
\end{table}

We note what the audit also establishes in the other direction: the pre-registered criterion was not
...
explanation was stated as a falsifiable prediction; the prediction was tested at real cost and
falsified; only then was the criterion amended, and the superseded criterion and its verdict were
retained verbatim beside the new one. Had the criterion been relaxed at the first failure, the
...
A reproduction that fails its pre-registered criterion has not necessarily failed. In the case
reported here the criterion was badly scaled, the mechanism usually blamed was absent, and the
residual was a cross-environment offset four orders of magnitude inside a defensible relative
tolerance. Benchmarks that ship an agreement criterion should declare it per metric, in that
metric's own units, and should treat its failure as the beginning of an investigation.

\section*{Data and code availability}
\todo{Point at the released artifact, the comparison JSON, the re-derivation script
(\texttt{recompare\_per\_metric.py}), and this note's table generator, once the repository's public
