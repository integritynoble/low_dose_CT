...
- [`pwm_dose_equivalence/`](pwm_dose_equivalence/) — pip-installable **v0.2.1 alpha** library. Modality-agnostic `signal_equivalence_credential` + percentile / DeLong / **BCa** estimators + sample-size pre-flight (incl. `bound_M` for unbounded metrics) + small-$n$ anti-conservativeness warning + content-addressed framework hash + Tr_ct/Tr_mri/Tr_pet operators + **`audit_credential` internal-consistency audit + machine-readable `CREDENTIAL_JSON_SCHEMA`** (v0.2.1; backs the R3-4 reading guide red-flag checklist with code). 116/116 tests (incl. 10 end-to-end integration tests + 35 credential-audit tests), 100 % line coverage on 378 statements. **Four tutorial notebooks** in [`pwm_dose_equivalence/notebooks/`](pwm_dose_equivalence/notebooks/) — three validated (CT AUC / MRI Dice / PET CR) + one extending-to-new-modality (Optical, R3-3).
