...
> **Status (D9 + 15, 2026-06-04):** manuscript at **v0.3 working draft**, 22 pp incl. 3-pp Supplementary S1 on the Bernstein correction ([`paper_draft/CHANGELOG.md`](paper_draft/CHANGELOG.md) v0.2 → v0.3 → v0.3-polish). v0.3 main pass (V3-1 .. V3-5) propagated the D9 + 13 theory landings into the manuscript text; v0.3 polish (V3-6 .. V3-9) added Supplementary S1, `theory/proofs/monotonicity.md`, the pinned-library-version fill, and the non-null power simulation. Theory side: [`theory/proofs/`](theory/proofs/) holds 6 v0.1+ writeups (estimator.md now at v0.4 after the D9 + 15 Dice + CR sims); backing experiments are now **four sims closing the per-modality metric trio** in [`experiments/estimator_coverage/`](experiments/estimator_coverage/) (AUC coverage + AUC power + Dice + CR for CT lung-nodule / MRI segmentation / PET phantom), plus the 6-credential cross-modality demo in [`experiments/cross_modality_consistency/`](experiments/cross_modality_consistency/). Library side: [`pwm_dose_equivalence/`](pwm_dose_equivalence/) v0.1.0 alpha — pip-installable, 60/60 tests, 100 % line coverage; the v0.2.0 roadmap now also includes a small-$n$ anti-conservativeness warning the CR sim surfaced. Real-cohort per-modality Results tables remain gated on WS-2 Phase 1 (CT) and Phase 3 (MRI / PET data integrations).
...
| 2.2 | §3 Estimator validity / coverage simulations | BLOCK | 1.5 | **done @ D9 + 13/14** — [`theory/proofs/estimator.md`](theory/proofs/estimator.md) v0.2 backed by 27-cell coverage sim (D9 + 13) + 24-cell non-null power sim (D9 + 14, V3-9) both in [`experiments/estimator_coverage/`](experiments/estimator_coverage/); estimator defaults recorded (percentile / DeLong-for-AUC / BCa opt-in); both BLOCK sub-deliverables of §3 now closed |
...
| Path | Purpose | Status |
|---|---|---|
| [`paper_draft/`](paper_draft/) | *Nature Methods* manuscript | **v0.3 working draft** (22 pp incl. 3-pp Supplementary S1; see [`paper_draft/CHANGELOG.md`](paper_draft/CHANGELOG.md)) |
...
| [`theory/proofs/`](theory/proofs/) | Theory-side decision writeups paired with the manuscript v0.2 / v0.3 commitments: `mri_mask.md` (§7), `pet_reduction.md` (§8), `composition.md` (§4), `estimator.md` (§3, **v0.2 with both coverage + power sims backing**), `sample_size.md` (§2, numerical table + the AUC `ε` correction that v0.3 V3-1 applied), `monotonicity.md` (§5, conjecture + counterexample sketch; empirical check gated on Phase 1 pilot) | **6 writeups landed at D9 + 13/14; manuscript-side propagation landed at D9 + 14** (V3-1 / V3-2 / V3-4 / V3-5 / V3-6 / V3-7 / V3-8 / V3-9 — see [`paper_draft/CHANGELOG.md`](paper_draft/CHANGELOG.md)). Sections §2 / §3 / §4 / §7 / §8 closed (all BLOCKs done); §5 partially closed (theory side; empirical check gated on Phase 1 pilot); §1 depth-pass still pending. |
...
| [`experiments/estimator_coverage/`](experiments/estimator_coverage/) | Two seed-reproducible simulations of the paired-bootstrap estimator: `coverage_sim.py` (27 cells, ~35 min, **under the null** — verifies CI coverage of true Δ) + `power_sim.py` (24 cells, ~25 min, **non-null shifts** — verifies verdict-distribution behaviour and statistical power). Both back the estimator-default decision and the cohort-sizing implication in `theory/proofs/estimator.md`. | **reproducible** — `results.json` + `power_results.json` committed; seed = 42 |
...
- [`experiments/estimator_coverage/`](experiments/estimator_coverage/) — two simulations of the paired-bootstrap estimator: 27-cell coverage sim across (AUC × n × CI variant) under the null, plus 24-cell non-null power sim (V3-9). Both back the `theory/proofs/estimator.md` decisions and the cohort-sizing implication.
