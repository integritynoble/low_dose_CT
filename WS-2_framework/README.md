...
| # | Task | Output | Status @ v0.3 |
|---|---|---|---|
| 3.1 | Implement `pwm_dose_equivalence.signal_equivalence_credential()` (CT validator) | Core library + CT example | **partial** — productionised package [`pwm_dose_equivalence/`](pwm_dose_equivalence/) at **v0.2.2 alpha** (modality-agnostic API + percentile / DeLong / **BCa** estimators + sample-size pre-flight with `bound_M` for unbounded metrics + small-$n$ anti-conservativeness warning + content-addressed framework hash + Tr_ct/Tr_mri/Tr_pet operators + `audit_credential` internal-consistency audit + machine-readable `CREDENTIAL_JSON_SCHEMA` + **`pwm-audit` console entry point** for non-Python callers); **128/128 tests (incl. 10 end-to-end integration tests + 35 credential-audit tests + 12 CLI tests)** at 100 % line coverage on 437 statements; real-data CT example gated on Phase 1 pilot |
...
- [x] **V3-12 Manuscript §Discussion "Limitations of the present work"** (this commit, D9 + 19, 2026-06-08): adds a new 5-paragraph subsubsection to §Discussion that surfaces empirical-validation gaps the existing "Failure modes of the framework" prose did not cover. (1) Synthetic-only validation, real-cohort Tables 2 / 3 / 4 still `\todo`. (2) Three modalities validated; extension surface untested independently. (3) v0.2 schema does not yet pin `method.code_hash` (L0.2.1 drift surfaced into the manuscript). (4) Reader-variability is a hard ceiling on attainable ε. (5) WS-1 v0.5 INDETERMINATE-dominated regime called out by name in the manuscript prose (previously only in proofs/estimator.md §4a, the reading guide, and the WS-1 cross-reference). **PDF: 22 pp → 24 pp.** Page-count references updated in WS-2 README status pin + Subfolders, CONTRIBUTING.md, CITATION.cff, reproduction_guide.md. Historical CHANGELOG entries recording past 22-pp landings left intact. No library code change.
- [x] **Governance artifacts: `CONTRIBUTING.md` + `CITATION.cff`** (commit `ff99fc9`, D9 + 19, 2026-06-08): new [`pwm_dose_equivalence/CONTRIBUTING.md`](pwm_dose_equivalence/CONTRIBUTING.md) — full contribution policy with two-tier sign-off rule (Tier A = two of three maintainers for `framework_hash.py` / `credential*.py` / `estimator.py` / `sample_size.py` / `operators.py`; Tier B = one maintainer for documentation / tests / tutorials / examples / API-additive). Sections: dev-env setup; PR workflow; versioning policy; schema-mutation discipline (6-step checklist); code style; testing floor; bug reporting; security disclosure window for credential-forgery vulnerabilities; placeholder maintainer list (TBA at v0.3 → v0.4 manuscript revision); license intent. **Closes the manuscript §software_rigor "contribution policy is published as CONTRIBUTING.md in the repository" claim.** New [`pwm_dose_equivalence/CITATION.cff`](pwm_dose_equivalence/CITATION.cff) (CFF v1.2.0) — software-citation metadata + `preferred-citation` pointing at the *Nature Methods* manuscript + framework spec SHA-256 in `identifiers`. Manuscript §software_rigor "Release cadence and governance" paragraph rewritten to name the actual paths, list the Tier-A / Tier-B module surfaces explicitly, and mention the schema-mutation discipline + security-disclosure window. Library README "License" section expanded to "Contributing" + "Citation" + "License". PDF rebuilt at 22 pp; no LaTeX errors. No library version bump — pure governance artifacts.
- [x] **Examples + standalone JSON Schema artifact** (commit `b369a50`, D9 + 19, 2026-06-08): new [`pwm_dose_equivalence/examples/`](pwm_dose_equivalence/examples/) directory — `valid_ct_lung_nodule.json` (clean PASS credential at the WS-1 v0.5 lung-nodule operating point, n_test = 500, ε = 0.05, DeLong) plus six broken variants in `failures/` (tampered_verdict, inverted_ci, missing_field as hard issues; unknown_framework_hash, undersized_pass, bca_headline as soft warnings) — one per audit signal. `examples/regenerate.py` issues every example deterministically; `examples/README.md` is the per-file audit-signal cheat-sheet. New top-level [`pwm_dose_equivalence/credential_schema.json`](pwm_dose_equivalence/credential_schema.json) — standalone JSON Schema dump of `CREDENTIAL_JSON_SCHEMA` for non-Python validators (JSON-Schema-aware editors, `ajv`, registry tooling). Regenerated from `scripts/dump_schema.py`. 11 new tests in `tests/test_examples.py` exercise every example and the schema artifact; drift detection compares re-issued examples against committed JSON. **128 → 139 tests at 100 % coverage on 437 statements** (statement count unchanged; new tests exercise existing code paths). Reading guide v1.2 → v1.3 (new §7b "Worked examples" with the per-file table). No library version bump — pure artifact addition.
- [x] **L0.2.2 `pwm-audit` CLI** (commit `903fa61`): library v0.2.1 → v0.2.2 — adds the `pwm-audit` console entry point (registered via `[project.scripts]` in `pyproject.toml`, pointing at `pwm_dose_equivalence.cli:main`). Same audit checks as v0.2.1's `audit_credential` but available from the shell with no Python call required: `pwm-audit credential.json` (human-readable), `pwm-audit credential.json --json` (machine-readable), `cat credential.json | pwm-audit -` (stdin). Exit-code contract: `0` if `ok=True`, `1` on any hard issue, `2` on I/O or argument error. Closes the "drop into a CI hook or submission-checklist script" use case for non-Python callers. **Test coverage 116 → 128 tests at 100 % line coverage (378 → 437 statements; +12 CLI tests).** Reading guide v1.1 → v1.2 (§7a documents both the Python API and the CLI). Manuscript §software_rigor Testing paragraph updated (116/378 → 128/437; CLI exit-code contract exercised), Versioning policy bumped to 0.2.2, §anchoring gains a sentence about the `pwm-audit` shell entry point.
- [x] **L0.2.1 `audit_credential` + `CREDENTIAL_JSON_SCHEMA`** (commit `bb620ef`): library v0.2.0 → v0.2.1 — adds `pwm_dose_equivalence.audit_credential(json_dict_or_str)` (one-call internal-consistency audit for a published credential: schema validity, framework-hash recognition, verdict self-consistency, sample-size-check coherence, BCa-as-headline / DeLong-nonzero-bootstrap warnings) returning a `CredentialAudit` dataclass; and `pwm_dose_equivalence.credential_schema.CREDENTIAL_JSON_SCHEMA` (machine-readable draft-07-compatible JSON Schema for the credential wire format — `json.dumps`-able to a standalone artifact). Backs the R3-4 reading guide's red-flag checklist and §7a "audit_credential shortcut" with code. **Test coverage 81 → 116 tests at 100 % line coverage (265 → 378 statements; +35 audit tests).** Reading guide reconciled from v1.0 to v1.1 (JSON example fixed to match the v0.2 schema's bare-string `method` field; schema-evolution note added explaining the v1.0 widening to `{name, code_hash}`). Manuscript §software_rigor Testing paragraph updated (60/230 → 116/378), Versioning policy bumped to 0.2.1; §anchoring gains an "Internal-consistency audit" paragraph anchoring `audit_credential` in the framework discussion.
- [x] **R3-4 Credential reading guide** (commit `f9780ec`): new [`paper_draft/credential_reading_guide.md`](paper_draft/credential_reading_guide.md) — 8-section walk through a published credential JSON for reviewers / regulators / clinicians who need to *interpret* a credential without re-running the bootstrap themselves. Covers verdict semantics (PASS / FAIL / INDETERMINATE), CI vs $\varepsilon$ reading, the framework-hash guarantees + non-guarantees (3 + 3), the `sample_size_check` field (with the WS-1 v0.5 INDETERMINATE-dominated regime called out by name at n ≈ 208 / AUC ≈ 0.92), 7 common reviewer / regulator questions, and a 9-entry red-flag checklist. Audience-disjoint from `reproduction_guide.md`: the reproduction guide answers "how do I re-derive the numbers?"; the reading guide answers "how do I read a credential someone else published?" `reproduction_guide.md` §10a + library README "Tutorial notebooks" section + manuscript §software_rigor "Documentation and tutorial notebooks" paragraph updated to point at both companion documents. **Closes the manuscript §software_rigor "Credential reading guide" claim**, leaving zero unbacked claims in that paragraph.
...
- [`pwm_dose_equivalence/`](pwm_dose_equivalence/) — pip-installable **v0.2.2 alpha** library. Modality-agnostic `signal_equivalence_credential` + percentile / DeLong / **BCa** estimators + sample-size pre-flight (incl. `bound_M` for unbounded metrics) + small-$n$ anti-conservativeness warning + content-addressed framework hash + Tr_ct/Tr_mri/Tr_pet operators + `audit_credential` internal-consistency audit + machine-readable `CREDENTIAL_JSON_SCHEMA` (v0.2.1) + `pwm-audit` console entry point (v0.2.2) + **worked examples** in `examples/` (one clean + six broken credentials, one per audit signal) + **standalone `credential_schema.json`** for non-Python validators (D9 + 19). 139/139 tests (incl. 10 end-to-end integration tests + 35 credential-audit tests + 12 CLI tests + 11 examples/schema-artifact tests), 100 % line coverage on 437 statements. **Four tutorial notebooks** in [`pwm_dose_equivalence/notebooks/`](pwm_dose_equivalence/notebooks/) — three validated (CT AUC / MRI Dice / PET CR) + one extending-to-new-modality (Optical, R3-3).
