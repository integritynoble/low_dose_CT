...
> **Status (D9 + 13, 2026-06-02):** manuscript at **v0.2 working draft**, 19 pp ([`paper_draft/CHANGELOG.md`](paper_draft/CHANGELOG.md)). Theory side: [`theory/proofs/`](theory/proofs/) ships v0.1 writeups of `mri_mask.md`, `pet_reduction.md`, `composition.md`, plus `estimator.md` (backed by a 27-cell coverage simulation in [`experiments/estimator_coverage/`](experiments/estimator_coverage/)) and `sample_size.md` (closed-form formulas + numerical tables; **surfaces a v0.3 manuscript correction** on the AUC-task default $\varepsilon$). Library side: [`pwm_dose_equivalence/`](pwm_dose_equivalence/) v0.1.0 alpha scaffold — pip-installable, 49/49 tests pass, **93 % line coverage** (above the manuscript's 90 % v0.2 target). Real-cohort per-modality Results tables remain gated on WS-2 Phase 1 (CT) and Phase 3 (MRI / PET data integrations).
