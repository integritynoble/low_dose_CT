...
**4a.2. At realistic operating points the framework PASSes cleanly under the null.** AUC = 0.92, $n = 500$ — close to the WS-1 1.0 cohort + a typical lung-nodule AUC operating point — gives P(`PASS`) = 0.94 under the null, matching nominal $1 - \alpha = 0.95$. The framework's coverage *and* its calibration both hold at this operating point.
...
The non-null table sharpens the §3 cohort-sizing implication: the WS-1 1.0 cohort of $n \approx 208$ is *exactly* in the regime where P(`PASS`) under the null is sensitive to the operating AUC. At AUC = 0.92 (the typical lung-nodule operating point), $n = 200$ gives P(`PASS`) = 0.40 under the null; expansion to $n \approx 500$ would lift this to $\approx 0.94$. Without that expansion, credentials issued at the 1.0 cohort should be expected to return `INDETERMINATE` rather than `PASS` even when the candidate is truly equivalent — *the absence of a `PASS` verdict is not evidence of non-equivalence*, only of insufficient $n$. This is the reading the manuscript v0.3 §methods-estimator paragraph and `proofs/sample_size.md` §5 jointly endorse.
