...
> preserved and extended. Tests: **253 total — current full-suite baseline (verified
> 2026-09-18): Windows 247 passed / 6 skipped; WSL Linux 243 passed / 10 skipped** — see
> SECTION1_LINUX_REVERIFICATION_2026-09-18.md. The per-section counts below (55/10, 78/68,
> 83/73, 93/83, 106/96) are **historical section-level counts** retained for provenance.
...
- `RUNG_REGISTRY.md` — rendered registry (workspace root: `WS-4_leaderboard/RUNG_REGISTRY.md`
  from the repo root; regenerated by `python -m scoring.cli rung-status` from
  `scoring/data/rung_registry.json`). **Current state (verified 2026-09-18): all six Rungs
  are `done`** — Rung 1 detectability-first baseline, Rung 2 real paired low-dose +
  geometric pairing, Rung 3 lesion model + noise-ROI protocol, Rung 4 dose-detectability
  curve, Rung 5 per-vendor/per-dose spread, Rung 6 cross-vendor generalization; the only
  per-rung remaining item is the domain-verifier independent recomputation noted in the
  registry rows.
...
```
python -m pytest scoring/tests -q        # historical section-level: 34 passed (full suite 2026-09-18: 253)
```
...
behaviour. Suite: 78 tests, 68 passed / 10 skipped, up from 65 / 55 (historical section-level counts; full-suite baseline 2026-09-18: 253).
...
regenerated. Suite: 83 tests, 73 passed / 10 skipped, up from 78 / 68 (historical section-level counts; full-suite baseline 2026-09-18: 253).
...
tests submission rather than seeding. Suite: 93 tests, 83 passed / 10 skipped, up from 83 / 73 (historical section-level counts; full-suite baseline 2026-09-18: 253).
...
skipped, up from 93 / 83 (historical section-level counts; full-suite baseline 2026-09-18: 253).
