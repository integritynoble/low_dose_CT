...
1. **Paired scoring gate (P0)** — the service receives fidelity (PSNR/SSIM) **and**
   detectability (CNR / CHO-AUC / NPWE) together; a submission without detectability is
   not publishable ("both numbers or neither", §4). The gate is a dependency-free
...
2. **Permanent Gaussian blur trap (P0)** — the `seed-blur` entry (sigma=1.0 px, 5x5 kernel,
   ...
   listing shows each entry's gap vs the blur (PSNR high, CNR < Rose 3).
...
| `verify.py` | `check_paired_submission` / `check_submission_result` — the §4 both-or-neither gate; accepts the WS-3 RunBundle `results.json` layout (`validation.paired_methods`) or a flat metrics dict. |
