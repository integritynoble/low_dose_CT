...
class T2T_module(nn.Module):
    ...
    def __init__(self, img_size=64, tokens_type='performer', in_chans=1, embed_dim=256, token_dim=64, kernel=32, stride=32):
        ...
        if tokens_type == 'transformer':
            ...
            # CTformer (Wang PMB 2023) convolution tokenization:
            # split into k x k patches (kernel=32, stride=32) -> 16x16=256 tokens,
            # linear-projected to embed_dim. Single-stage, no token-transformer stages.
            self.soft_split0 = nn.Unfold(kernel_size=(kernel, kernel), stride=(stride, stride))
            self.project = nn.Linear(kernel * kernel * in_chans, embed_dim)
    ...
    def forward(self, x):
        if self.tokens_type == 'convolution':
            x = self.soft_split0(x)             # (B, K*K*C, N)
            x = self.project(x.transpose(1, 2)) # (B, N, embed_dim)
            return x, None, None
...
class Token_back_Image(nn.Module):
    ...
    def __init__(self, img_size=64, tokens_type='performer', in_chans=1, embed_dim=256, token_dim=64, kernel=32, stride=32):
        ...
        if tokens_type == 'transformer':
            ...
            # Mirror of T2T_module convolution branch: embed_dim -> K*K*C, Fold back to image.
            self.project = nn.Linear(embed_dim, kernel * kernel * in_chans)
            self.soft_split0 = nn.Fold((img_size, img_size), kernel_size=(kernel, kernel), stride=(stride, stride))

        self.num_patches = (img_size // (1 * 2 * 2)) * (img_size // (1 * 2 * 2))  # there are 3 sfot split, stride are 4,2,2 seperately

    def forward(self, x, res_11,res_22):    
        if self.tokens_type == 'convolution':
            x = self.project(x).transpose(1, 2)     # (B, K*K*C, N)
            x = self.soft_split0(x)                 # (B, C, H, W)
            return x
