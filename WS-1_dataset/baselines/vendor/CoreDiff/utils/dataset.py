...
from .dataset_lidc import LIDCV05Dataset
...
def _train_builder(dataset='mayo_2016_sim', test_id=9, dose=25, context=True):
    if dataset == 'lidc_v05':
        return LIDCV05Dataset(mode='train', dose=dose, context=context)
    return CTDataset(dataset=dataset, mode='train', test_id=test_id, dose=dose, context=context)


dataset_dict = {
    'train': _train_builder,
    ...
    'lidc_v05': partial(LIDCV05Dataset, mode='test', dose=25, context=True),
}
