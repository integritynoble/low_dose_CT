"""Task-based detectability observers for PWM-LDCT v0.5 (Rung 1 / §4).
...
Protocol (insertion study): a synthetic Gaussian lesion is inserted into the
low-dose INPUT slice (the lesion is part of the anatomy, so it is present in
both the low-dose acquisition and the full-dose reference). The model
reconstructs both the lesion-present and lesion-absent inputs; detectability is
measured on the model OUTPUTS. A method that merely smooths (e.g. Gaussian
blur) destroys the inserted signal, so its CNR falls below the Rose criterion
(3.0) even when PSNR rises -- the blur trap (Rung 1.3 / §4).

The noise ROI is selected to be a tissue-only patch (HU band + low gradient),
never placed over anatomy (Rung 3). Observer parameters are stated and fixed
(Rung 1.2).
"""
...
def hu_to_norm(hu: np.ndarray) -> np.ndarray:
    """Normalise HU to [0, 1] (matching pwm_ldct_baselines.data.normalize)."""
...
def run_model(model, x):
    """Run a model for inference, handling tuple outputs and 1ch->3ch fallback.

    Some registered models (e.g. CoreDiff bridge) return (recon, recon_sub1)
    or expect 3-channel context; this adapter normalises to a single [B,1,H,W]
    tensor so the detectability protocol and the fidelity loop share one path.
    """
    try:
        out = model(x)
...
class TaskSpec:
    ...
    All fields are fixed and published with the benchmark.
    """
    ...
    def to_dict(self) -> dict:
        """Serializable mirror of the published task spec (baselines/task_spec.json)."""
        return {
            "task": self.label,
            "signal_sigma_px": self.signal_sigma_px,
            "peak_contrast_hu": self.peak_contrast_hu,
            "roi_size_px": self.roi_size_px,
            "location_known": self.location_known,
            "noise_roi_hu_band": [float(v) for v in self.noise_roi_hu_band],
    ...
    def from_dict(cls, d: dict) -> "TaskSpec":
        return cls(
            signal_sigma_px=float(d.get("signal_sigma_px", 2.0)),
            peak_contrast_hu=float(d.get("peak_contrast_hu", 20.0)),
            roi_size_px=int(d.get("roi_size_px", 32)),
            location_known=bool(d.get("location_known", True)),
            noise_roi_hu_band=tuple(float(v) for v in d.get("noise_roi_hu_band", [10.0, 120.0])),
            n_trials=int(d.get("n_trials", 64)),
            label=str(d.get("task", "SKE-Gaussian20HU-s2px")),
        )
...
def find_tissue_roi(img_norm: np.ndarray, patch_size: int,
    ...
    gradient, and low std. Returns (row, col) of the top-left corner.
    """
    if task is None:
        task = TaskSpec()
    h, w = img_norm.shape
...
# ---------------------------------------------------------------------------
# Per-slice model detectability
# ---------------------------------------------------------------------------

def model_detectability(model: torch.nn.Module, low_norm: np.ndarray,
                        full_norm: np.ndarray, task: TaskSpec,
                        device: str = "cpu", roi_pos: Optional[Tuple[int, int]] = None,
                        n_trials: Optional[int] = None) -> dict:
    """Run the insertion protocol for one slice through the model.

    Parameters
    ----------
    model : torch.nn.Module
        Reconstruction model (low-dose -> full-dose), eval mode, [0,1] in/out.
    ...
    def _run(img: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            return run_model(model, x).clamp(0, 1)[0, 0].cpu().numpy()

    out_present = _run(low_present)
    out_absent = _run(low_norm)
...
def evaluate_detectability(model: torch.nn.Module,
    ...
    """Batch model detectability over multiple slices.

    The tissue ROI is chosen once from the first reference slice and reused
    (same anatomical location across the cohort's slices, avoiding anatomy).
    """
    if task is None:
        task = TaskSpec()
    ...
    roi_pos = find_tissue_roi(full_norm_batch[0], task.roi_size_px, task)
    cnrs, aucs, npwes = [], [], []
    for low, full in zip(low_norm_batch, full_norm_batch):
        det = model_detectability(model, low, full, task, device=device,
                                  roi_pos=roi_pos, n_trials=n_trials)
        cnrs.append(det["cnr"])
    ...
    return {
        ...
        "cho_auc_values": [float(v) for v in aucs],
        "npwe_mean": float(np.mean(npwes)),
        "npwe_values": [float(v) for v in npwes],
        "n_slices": len(cnrs),
        "task_label": task.label,
