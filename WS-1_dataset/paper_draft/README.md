...
- **Vendor coverage** (corrected 2026-05-26): the public Mayo LDCT-PD cohort is actually **two-vendor — ~99 GE + ~101 Siemens** (verified from TCIA metadata + a GE projection file), so the earlier "Siemens-only" claim was a factual error. The manuscript now reports the GE/Siemens split accurately and **promotes cross-vendor (GE↔Siemens) real paired-dose coverage as a v0.5 strength**; v1.0's distinction is re-scoped to prospective + multi-site + additional vendors (Canon/Philips) + new tasks. (A separate open item: AAPM/Mayo low-dose is generated by projection-domain noise insertion, not a second physical scan — the "real ... acquisitions" wording elsewhere should be tightened to reflect that.)
