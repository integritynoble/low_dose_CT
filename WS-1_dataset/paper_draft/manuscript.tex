...
PWM-LDCT v0.5 is released alongside the signal-equivalence framework~\citep{ws2framework}, an open-source unrolled-iterative reference reconstruction method~\citep{ws3reference}, and the PWM Low-Dose CT Challenge leaderboard~\citep{ws4leaderboard}. The four releases together form a coherent substrate for citation-standard low-dose CT method evaluation.
...
For 5-tuple credential evaluation per the signal-equivalence framework~\citep{ws2framework} at canonical target $(\varepsilon = 0.02, \alpha = 0.05)$ on AUC-type metrics with conservative $\sigma \approx 0.10$ and pairing correlation $\rho \approx 0.5$, the leading-order sample-size requirement is $n \geq 192$ patients per evaluation slice. The v0.5 test split (~265 patients) supports credentialed evaluation on the aggregate chest-nodule-detection task. Sub-population evaluations (real-paired-only, AAPM-only, Mayo-only) are limited by their underlying subset size; we report these explicitly in the credential reporting and refuse to issue PASS verdicts when the underlying sample size falls below the formula's requirement.
...
We trained four community-standard reconstruction baselines on the v0.5 training split and report their test-split performance in Table~\ref{tab:baselines_v05}. The baselines are RED-CNN~\citep{chen2017redcnn}, a transformer-based method (\todo{select}), a diffusion-based method (\todo{select}), and the open-source PWM reference unrolled-iterative reconstruction~\citep{ws3reference}. The objective is not to declare a winner but to (i) confirm that all baselines successfully train and evaluate on the v0.5 unified loader, and (ii) provide initial seed entries for the leaderboard at launch.
...
For each baseline we report the corresponding 5-tuple credential~\citep{ws2framework} for the canonical chest lung-nodule detection task at $r = 0.25$. Credentials are restricted to subpopulations that meet the framework's sample-size requirement; per-source-stratified credentials for the AAPM-only and Mayo-only subsets are reported explicitly as \texttt{INDETERMINATE} when underpowered. The full credential set is in Table~\ref{tab:credentials_v05}; per-method credential JSON ships in the release.

\todoTable{tab:credentials_v05}{5-tuple signal-equivalence credentials for four baselines at canonical lung-nodule detection, $r = 0.25$, on v0.5 chest test split. INDETERMINATE verdicts are honest reporting where underlying sample size is insufficient.}
...
Submissions to the PWM Low-Dose CT Challenge leaderboard~\citep{ws4leaderboard} take the form of a PWM RunBundle (Docker image + eval entry point + \texttt{results.json} + 5-tuple credentials~\citep{ws2framework}). The Year-1 leaderboard runs against PWM-LDCT v0.5; the v1.0 release will be added as an extended L3 spec when it ships, and submissions can be evaluated against either version.
...
\begin{enumerate}
    ...
    \item \textbf{Credentialed leaderboard submission.} Methods that compute 5-tuple credentials per~\citep{ws2framework} against v0.5 produce content-addressed, third-party-verifiable evaluation evidence rather than free-text claims.
\end{enumerate}
