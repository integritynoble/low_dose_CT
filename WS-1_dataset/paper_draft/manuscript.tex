...
%% --- TODO macro for placeholders to fill in with real data ---
...
%% =====================================================================
\begin{abstract}
\noindent We present \textbf{PWM-LDCT}, a multi-vendor, multi-site, paired-dose chest- and abdominal-CT dataset designed as a permanent, reproducibility-first benchmark for low-dose CT reconstruction research. The dataset comprises \Nscans{} paired scans (full-dose and matched low-dose, with low-dose obtained both from real reduced-tube-current acquisitions on \NvendorA{} patients and from physically calibrated Poisson-noise simulation on the full-dose data of \NvendorB{} patients), covering at least two manufacturer platforms (\todo{vendor list}) across \todo{N$_{\textrm{sites}}$} clinical sites. Every scan ships with raw projections (sinograms), reconstructed images at four dose levels (10\%, 25\%, 50\%, 100\%), majority-vote radiologist annotations for lesion detection and segmentation tasks, and full provenance metadata. The release is distributed under PhysioNet credentialed access, with a content-addressed hash registered as an immutable L3 specification on the PWM blockchain registry so that any future submission to the PWM Low-Dose CT Challenge leaderboard can be cryptographically tied to a specific frozen benchmark version. We further publish a pip-installable Python loader (\texttt{pwm\_ldct\_loader}) and three reproducible Docker pipelines so that loading, preprocessing, and evaluation are reproducible bit-for-bit across machines. By foregrounding multi-vendor diversity, paired real low-dose acquisitions (rather than simulation alone), task-level annotations, and on-chain immutability, PWM-LDCT addresses three long-standing gaps in the low-dose CT literature: vendor specificity of published methods, evaluation through PSNR alone rather than clinical task performance, and benchmark drift over time. We anticipate the dataset will serve as the substrate for the recurring PWM Low-Dose CT Challenge at RSNA / ISBI and as the empirical anchor for the signal-equivalence framework~\citep{ws2framework}.
\end{abstract}
...
All scans were de-identified per HIPAA Safe Harbor (45~CFR~164.514(b)(2)). The DICOM tag whitelist and the burned-in-text scrubbing recipe are published as \texttt{schema/dicom\_cleaning\_spec.md} in the project repository. We verified de-identification on every scan via a two-stage pipeline: (i) automated PyDICOM tag stripping against the whitelist; (ii) optical character recognition (OCR) sweep of every reconstructed slice with manual review of any positive hits. No PHI leakage was identified in the released set.
...
On the \NvendorA{} patients with paired real-low-dose acquisitions, we measured the per-patient \textit{distributional} agreement between the simulated 25\%-dose image (generated from the patient's own full-dose sinogram) and the real 25\%-dose acquisition. The agreement metrics are reported in Figure~\ref{fig:sim_vs_real} \todo{generate}. The purpose is not to assert simulation equivalence (it is well known that simulated low-dose noise underestimates real low-dose noise in some regimes~\citep{wang2020deeplearningreview}) but to make the discrepancy visible so users can decide which split is appropriate for their evaluation.

\subsection*{Inter-rater annotation reliability}
Cohen's $\kappa$ for lung nodule detection (per-radiologist, per-scan) and Dice for liver lesion segmentation are summarized in Table~\ref{tab:irr} \todo{populate}. We report agreement broken down by dose level so that any tendency of low-dose images to depress annotator agreement is visible.

\subsection*{Baseline reproductions}
We trained four community-standard baselines on the PWM-LDCT training split and report their test-split performance in Table~\ref{tab:baselines} \todo{populate after training}. The baselines are RED-CNN~\citep{chen2017redcnn}, a transformer-based method (\todo{select}), a diffusion-based method (\todo{select}), and the PWM reference unrolled iterative reconstruction~\citep{ws3reference}. The objective of this section is not to declare a winner but to (i) confirm that all baselines successfully train and evaluate on the released dataset using the provided loader, and (ii) provide initial seed entries for the permanent leaderboard.

\subsection*{5-tuple credentials per signal-equivalence framework}
For each baseline we report the corresponding 5-tuple signal-equivalence credential~\citep{ws2framework} for the canonical lung nodule detection task at 25\% dose. The credentials are summarized in Table~\ref{tab:credentials} \todo{populate}; the per-method credential JSON is provided in the release as \texttt{baselines/<method>/credentials.json}.
...
\begin{verbatim}
    ...
    train = LowDoseCTDataset(root="/path/to/pwm_ldct",
                              ...
                              seed=42)
    sample = train[0]
    # sample["full_dose"]   -> torch.Tensor [H, W]
    # sample["low_dose"]    -> torch.Tensor [H, W] (real if available, else simulated)
    # sample["sinogram"]    -> torch.Tensor [n_views, n_dets]
