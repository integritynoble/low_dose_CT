...
\begin{table}[ht]
    ...
    \begin{tabular}{lcccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Patient-level demographics per source. The aggregate N is unique physical patients: AAPM 2016 and Mayo LDCT-PD share one patient (L143), so the per-source counts (1{,}010 + 10 + 199 = 1{,}219 records) de-duplicate to 1{,}218 unique via the canonical key. Age, sex, BMI, and scan-date fields are aggregated from each source's published metadata where available; \todo{} entries are populated at submission from the source DICOM headers. Fields not preserved in a source's de-identified release are recorded as N/A. Geographic origin captures the population from which each cohort is sampled and bears on generalizability claims.}
