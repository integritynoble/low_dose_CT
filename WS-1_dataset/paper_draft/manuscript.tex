...
\begin{table}[h]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        \textbf{Total}                  & \textbf{\todo{$\sim$731}} & \textbf{\todo{$\sim$243}} & \textbf{\todo{$\sim$252}} & \textbf{1{,}226} \\
        \bottomrule
    \end{tabular}
    \caption{v0.5 cohort composition by source and split. Splits are patient-level, stratified by source and anatomy. Counts in parentheses are deterministic given the random seed; small adjustments may occur at release to ensure each split has at least one patient from every source-anatomy stratum. Per-source rows are source records; the AAPM 2016 split shown is the deposited assignment (its one Mayo-shared patient, L143, is co-located in train). Totals are unique physical patients --- the 1{,}227 source records (1{,}018 + 10 + 199) de-duplicate to 1{,}226 unique via the canonical key.}
...
\begin{table}[ht]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        N (patients, unique)                          & 1{,}018          & 10               & 199              & 1{,}226 \\
        ...
        \bottomrule
    \end{tabular}
    \caption{Patient-level demographics per source. The aggregate N is unique physical patients: AAPM 2016 and Mayo LDCT-PD share one patient (L143), so the per-source counts (1{,}018 + 10 + 199 = 1{,}227 records) de-duplicate to 1{,}226 unique via the canonical key. Age, sex, BMI, and scan-date fields are aggregated from each source's published metadata where available; \todo{} entries are populated at submission from the source DICOM headers. Fields not preserved in a source's de-identified release are recorded as N/A. Geographic origin captures the population from which each cohort is sampled and bears on generalizability claims.}
...
\paragraph{Source overlap (AAPM 2016 and Mayo LDCT-PD).} The 10 AAPM 2016 challenge cases are drawn from the same Mayo patient population as the Mayo LDCT-and-Projection-Data collection. Reconciling the two by native patient identifier, \textbf{exactly one patient (Mayo ID L143) appears in both sources} (as an AAPM chest record and a Mayo abdomen record). We treat the two as nominally distinct sources but de-duplicate at the patient level via the cross-source canonical key (Methods, \S Patient-level data splits): the shared patient's records are co-located in a single split (train) --- preventing train/test leakage --- and the patient is counted once, so the 209 paired-dose source records resolve to 208 unique patients and the 1{,}227-record union to 1{,}226 unique. Both source records remain available; only the patient-level count is de-duplicated.
