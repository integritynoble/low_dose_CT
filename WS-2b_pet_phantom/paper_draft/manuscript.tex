...
4:1 (design-locked in Phase~A), together with (i) genuinely reduced-activity acquisitions
...
This descriptor is the \textbf{data-descriptor companion to the WS-2
signal-equivalence framework}~\citep{ws2framework}: its purpose is to provide a
physically ground-truth-anchored phantom substrate for \emph{validating a
dose-reduction (digital-twin) model} --- specifically, whether list-mode Poisson
thinning at rate $r$ faithfully reproduces a genuinely reduced-activity PET
acquisition of the same geometry, and how a fixed reconstruction preserves known
physical contrast as counts decrease. It is \textbf{not} a CT detectability
benchmark and it is \textbf{not} Rung~1 evidence for any reconstruction method:
no lesion-detectability claim (AUC, channelized-observer, or reader study) is
made anywhere in this descriptor, and the dataset's endpoints (NEMA contrast
recovery, background variability, contrast-to-noise, decayed-vs-thinned
agreement) measure \emph{signal fidelity against physical ground truth}, not
decision-task performance. The phantom task and observer plan are defined
explicitly in \texttt{analysis/task\_spec.json} /
\texttt{analysis/TASK\_DEFINITION.md} (Rung~1.1 task-stated discipline) and are
summarized in Technical Validation. Readers should treat any statement that this
dataset ranks or validates detectability as a misreading of the release's scope.
...
(\todo{radiation-safety authorization no.}). The acquisition is an
\emph{irreversible embodied act} (one phantom fill yields the decayed sequence
exactly once), so it is performed only under a named grant: the site PI /
radiation-safety office and the PWM Protocol Foundation deposit owner authorize
the exact act --- one fill, reference scan, and decayed series at
$r \in \{1.0, 0.50, 0.25, 0.10\}$ on the designated scanner --- and any change
requires a grant amendment. The grant record, the raw-data hosting lineage
(secured scratch $\rightarrow$ processing container $\rightarrow$ PhysioNet /
Zenodo, read-only raw files, two independent copies), and the solved-when gates
are maintained in \texttt{ACQUISITION\_AUTHORIZATION.md}; no scanning begins
until that file's signature block is complete. The dataset is released under
...
hot 10--22\,mm / cold 28--37\,mm (design-locked in Phase~A;
\texttt{analysis/task\_spec.json}). Sphere and background fill activity
concentrations were prepared to a sphere-to-background ratio of 4:1
(design-locked in Phase~A; \texttt{analysis/task\_spec.json}),
...
in this skeleton. The phantom task and observer plan follow Rung~1.1
task-stated discipline: signal = the six known spheres (10--22\,mm hot, 28--37\,mm
cold) at 4:1 sphere-to-background ratio; background = the NEMA 12-region ROI set;
location = known by construction; observer = NEMA NU~2-2018 numeric metrics
(percent contrast recovery, background variability, contrast-to-noise) computed by
the committed \texttt{analysis/nema\_metrics.py}, with \emph{no} reader panel and
\emph{no} channelized-observer detectability experiment in 1.0 --- contrast recovery
measures signal fidelity, not task detectability, and the descriptor makes no
detectability claim (see \texttt{analysis/TASK\_DEFINITION.md} and the Scope and
positioning section).
...
scanner with one tracer at one sphere-to-background ratio (4:1, locked in
Phase~A). Multi-scanner,
