...
> **Dated amendment 2026-09-11 — WS-1 document refresh (repo HEAD `cb88439` on `heyang`).** This page
> is retained as the 2026-09-04 snapshot. Nothing on it has been closed by this note — every decision
> below is still the Director's. Four factual corrections, so the WS-1 items are not read stale:
> - **§1.2 / §1.4 wording.** WS-1's submission manuscript now carries **0 `\todo{`**; the IRB and
>   competing-interest slots are prose, not macros (see the notes added in those sections).
> - **§2.1 (R6 tolerance).** The route-(a) deterministic re-run has been performed (2026-09-05/06) and
>   its evidence appended to `R6_recalc/results/comparison_full764.json`, which also now carries a
>   per-metric declaration; `R6_recalc/compare_full764.py` **is** committed, so the "not in the repo"
>   note below is stale. The JSON's `"overall"` field still reads `FAIL`.
> - **WS-1 table provenance.** `tab:harmonization_v05` now uses a **150 HU** Wasserstein bound with a
>   worst observed vendor pair of **144.9 HU (Philips–Toshiba)**, regenerated by the committed
>   `analysis/extract_harmonization_table.py`; the previously published 200 HU / 199.3 HU pair and the
>   uncommitted `hv_fast.py` it traced to are superseded (recorded, with dates, in
>   `WS-1_dataset/SUBMISSION_CHECKLIST.md` §5, refreshed 2026-09-11).
> - **Repo anchor.** This page was written against `86cce03`; the WS-1 items should be read against the
>   current `heyang` HEAD `cb88439`.
...
Do **not** let the manuscript assert a determination whose number is not in hand — `manuscript.tex` currently carries `\todo{IRB determination no.}`, which is the honest state.

> **Dated amendment 2026-09-11.** The `\todo{}` macro is gone (the submission manuscript has
> 0 `\todo{`), but this did not close the item — it changed its shape. `manuscript.tex`
> §"Ethics, consent, and data sharing" (L142–148) now **asserts an exemption basis in prose**: v0.5
> "does not require a new IRB determination or informed consent", grounded in a three-part public-data
> basis (public-source data only; no PHI, re-verified by DICOM-tag whitelist + OCR sweep; no
> human-subjects interaction) and 45 CFR 46. So the paper is no longer waiting for a number to paste
> in — it is making a claim that wants your sign-off, and any institutional letter you do obtain
> should be attached as originally instructed below.

Attach the actual letter.
...
Currently not a `\todo` — but still not a disclosure. *(Dated amendment 2026-09-11: `manuscript.tex`
`\section*{Competing interests}` (L743–745) now states the PWM Protocol Foundation affiliation
("One or more authors are affiliated with the PWM Protocol Foundation, which contributed the
open-source registry tooling used in preprocessing") and commits that "at submission, each author will
declare any financial interest in the PWM Protocol Foundation (equity, consulting, or advisory role)
or state its absence." That is a **promise to disclose per author**, not a filled disclosure: the
per-author declaration is still missing, and WS-4's prize-pool/funding interest remains undisclosed.)*

At a medical venue an undisclosed financial interest in an entity the work promotes is the kind of thing that gets a paper **retracted rather than rejected**. It costs nothing to disclose and is not negotiable.
...
> **Dated amendment 2026-09-11.** Both requests are now partly satisfied: `R6_recalc/compare_full764.py` **is** committed, and `R6_recalc/results/comparison_full764.json` carries (i) the route-(a) deterministic re-run evidence (performed 2026-09-05/06 on 2×RTX 4090, `cudnn.deterministic=True`, `use_deterministic_algorithms(True)`; evidence dated 2026-09-09, issue #5) and (ii) a per-metric declaration block. The normative criterion itself is unchanged — `tolerance: 1e-6`, `tolerance_kind: "absolute"` — and `"overall"` still reads **`FAIL`**, so the per-metric misfire this paragraph warns about has not been closed by the declaration.
